...
exports.build = function (_a) {
    var cls = _a.cls, data = _a.data, id = _a.id;
    ...
    if (!data) {
        return [Buffer.from([cls, packetId, 0, 1])];
    }
    else if (data.length < DATA_SIZE) {
        return [Buffer.concat([Buffer.from([cls, packetId, 0, 1]), data])];
    }
    var total = Math.ceil(data.length / (BUF_SIZE - HEADER_SIZE)) || 1;
    var bufs = [];
    for (var i = 0; i < total; i++) {
        var header = Buffer.from([cls, packetId, i, total]);
...
exports.collect = function (data) {
    var cls = data.readUInt8(0);
    var id = data.readUInt8(1);
    var index = data.readUInt8(2);
    var total = data.readUInt8(3);
    var packet = { cls: cls, id: id, index: index, total: total, data: data.slice(HEADER_SIZE) };
    if (index === 0) {
        cache[id] = [packet];
    }
    else if (cache[id]) {
