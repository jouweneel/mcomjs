...
export const build = ({
  ...
  if (!data) {
    ...
    return [Buffer.concat(
      [ Buffer.from([ cls, packetId, 0, 1 ]), data ]
    )];
  }
  ...
  for (let i = 0; i < total; i++) {
    const header = Buffer.from([ cls, packetId, i, total]);
...
export const collect = (data: Buffer): XudpMessage => {
  const cls = data.readUInt8(0);
  const id = data.readUInt8(1);
  const index = data.readUInt8(2);
  const total = data.readUInt8(3);

  const packet: Packet = { cls, id, index, total, data: data.slice(HEADER_SIZE) };
